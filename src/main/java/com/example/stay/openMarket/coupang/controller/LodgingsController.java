...
public class LodgingsController {
    ...
    private static String omk = "COUPANG";

    /**
     * 숙박상품 생성
     */
    @GetMapping("/createLodging")
    public void createLodging(int con_id){
        try {
            String strPath =  Constants.LodgingsPath + "/v1/marketplace/travel/lodgings";

            // 숙박상품 생성에 필요한 데이터 가져오기
            // 숙박 상품 정보
            CondoDto roomInfo = commonApiService.getInfo(con_id, omk);
            if(roomInfo != null){
                // 룸타입별 정보
                List<ToconDto> roomType = commonApiService.getroomType(con_id, omk);
                // 취소규정
                String cancelInfo = commonApiService.getCancelInfo(con_id);
    ...
    //                        roomImages.put("sellerUrl", Constants.toconSellerUrl + t.getConId() + "/rooms/" + t.getPyongIdx() + "/" + pyongImgs[j]);
                ...
                // rates -----------------------------------------------------------------------------------------------
                JSONObject rates = updateLodgingRate(con_id, roomInfo, pyongIdxs, cancelInfo);
    ...
    //            System.out.println("**********************************");
                ...
                /**
                 * 숙박상품 생성 성공하면 OMK_PRODUCT에 coupang_id UPDATE --> cid(sellerProductId)도 바뀌는지 테스트해서 확인해보고 바뀌면 update쳐야함
                 * 응답값으로 rateId도 오는데 이건 db에 업데이트 안치는지? -> 해야지..
                 *
                 * 응답값으로 오는거
                 *
                 * travelProductId ->  OMK_PRODUCT의 coupang_id ㅇㅇ
                 * travelItemId -> tocon의 travelItemId ㅇㅇ
                 * rateId -> tocon의 rateId & (CP_PYONG_RATE의 rateid -> CP_PYONG_RATE는 업데이트 안하는데?) ㅇㅇ
                 *
                 * ----내가 보내는 값에 들어있음 -> 응답값이 내가 보낸거랑 다르게 왔는지 확인 필요할듯?
                 * sellerProductId -> 시설코드 con_id ㅇㅇ
                 * sellerRoomId -> pyong_idx ㅇㅇ
                 * sellerRateId -> pyong_idx ㅇㅇ
                 */
                ...
                System.out.println("상품 생성 불가능. 해당 상품의 재고 혹은 오픈마켓yn값 확인 필요");
            }
     ...
     * 숙박상품 수정
     */
    @GetMapping("/updateLodging")
    public void updateLodging(int con_id){
        String strPath =  Constants.LodgingsPath + "/v1/marketplace/travel/lodgings";
    ...
    public void creUpdRoom(int con_id){
        //roomtype 생성인지 수정인지 확인 -> travelItemId가 있으면 수정, 없으면 생성
        String strMethod = "";
        boolean createRoomOk = false;
        try{
            // 쿠팡에 시설이 생성되어있는지 확인
            String coupangId = lodgingsService.getCoupangIdByconId(con_id);
            if(coupangId != null && !coupangId.equals("")){
                ...
                List<ToconDto> creUpdRoomType = lodgingsService.getCreUpdRoomByConId(con_id);
    ...
    /**
     * 숙박상품 조회 -> 변수명 cid(con_id) -> 이거로 coupang_id 조회해서 작업
     */
    ...
    /**
     * 숙박상품 삭제 -> 변수명 cid(con_id)
     */
    @GetMapping("/deleteLodging")
    public void deleteLodgings(){
        // 임의로 박아둔것. 나중에 빼고 매개변수로 받기
        String travelProductId = "512542156641013763";
        ...
        try{
            ...
            long APIStart = System.currentTimeMillis();
            coupangApi.coupangDeleteApi(strPath);
            System.out.println("쿠팡 API 호출 실행 시간 : " + (System.currentTimeMillis()-APIStart)/1000.0);
            System.out.println("숙박 상품 삭제 완료");
        }catch (Exception e){
            e.printStackTrace();
            System.out.println("API 호출 실패");
        }
    ...
    public JSONObject updateLodgingRate(int con_id, CondoDto roomInfo, List<Integer> pyongIdxs, String cancelInfo){
        JSONObject lodgingRateUpdateDto = new JSONObject();
        try{
            ...
            String rateId = lodgingsService.getRateIdByConId(con_id);
            // 쿠팡 api 호출
